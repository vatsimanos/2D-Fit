...
void Steuerung(char *setfilename, short histopts, double koop, bool test)
{
  ...
  if (Setfile.channel_info.size() > 0)
  {
    ...
    if (!test)
    {
      ...
      // std::cout << "Setfile.sigma: " << Setfile.sigma << std::endl;
...
void Steuerung(char *setfilename, char *dwell, short histopts, double minevents)
{
}
...
void Copy_Matrix(TRate_matrix &DestMatrix, TRate_matrix SourceMatrix, short n_states)
{
  short i, j;
...
Next_line(std::ifstream &fhsetfile, char *line, short &line_counter, bool killcomments = true)
{
  ...
  do
  {
    ...
    if (killcomments)
    {
      char *x = strchr(dum, '#');
      if (x != NULL)
        *x = 0; // kommentare abschneiden
    }

    for (i = short('0'); i <= short('9'); i++)
    {
      if (strchr(dum, char(i)) != NULL)
        gefunden = true;
    }
...
bool Read_Matrix(std::ifstream &fhsetfile, TRate_matrix &m, short &line_counter, short &dim)
{
  char *pline;
  char line[256];
  short i, j;
  ...
  if ((dim1 != 0) && (dim1 != 0))
  {
    ...
    for (i = 1; i <= round(dim1); i++)
    {
      ...
      for (j = 1; j <= round(dim2); j++)
      {
        ...
        if ((i > n_states_max) || (j > n_states_max))
          break; // Vorsichtsma�nahme gegen Zugriffe auserhalb des Arrays
        m.r[i - 1][j - 1] = r;
      }
    }
    return true;
...
bool Read_Row(std::ifstream &fhsetfile, double row[], short &line_counter)
{
  char *pline;
  char line[256];
  short i;
...
void Read_Setfile(char *setfile_name, TSetFile &setfile)
{
  ...
  if (setfile.version == 1)
    setfile.no_channel_types = 1;
