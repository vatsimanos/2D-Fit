...
//**************************************************************************************************************************
double simulation(int multiplikator, int nr_const, double *rconst, double rtol, double sigma_noise)
{
  ...
  for (ii = 1; ii <= counts; ii++)
  {
    ...
    {
      Daten.a_fit.i_null = Daten.Geneticfitparameter.level1 + Setfile.get_zero_current();
      Daten.a_fit.i_channel = Daten.Geneticfitparameter.level2;
      Daten.a_fit.sigma = Daten.Geneticfitparameter.std_of_noise;
    }
    else
    {
      sigma_noise = Daten.a_fit.sigma;
    }
    ...
    Setfile.simulate_timeseries(&TobyTimeseries, &Rausch, (char *)"", 0, sigma_noise);
    ...
    // std::cout << "Daten.a_fit.i_channel: " << Daten.a_fit.i_channel << std::endl;
    // std::cout << "Daten.a_fit.sigma: " << Daten.a_fit.sigma << std::endl;
    if (Daten.Geneticfitparameter.levelfit == false)
    {
      Daten.hinkley(_HOHD);
    ...
    {
      // std::cout << "Daten.a_fit.null: " << Daten.a_fit.i_null << " ";
      // std::cout << "Daten.Wert: " << Daten.Wert[10000] << " ";
      // std::cout << "Daten.a_fit.channel: " << Daten.a_fit.i_channel << " ";
      // std::cout << "Daten.a_fit.std_of_noise: " << Daten.a_fit.sigma << std ::endl;
  ...
  // double temp = 0;
  for (i = 0; i < 65536; i++)
  { // 2014 Test level default 0 -> <65536
  ...
  if (Daten.Geneticfitparameter.fit_methode == 3)
    ...
    {
      // std::cout<<"likelihood_2d :"<<likelihood_2d<<std::endl;
      // atd::cout<<"histogram_diff :"<<histogram_diff<<std::endl;
  ...
  if (Daten.SimSettings.dosbox)
  {
    ...
    if (Daten.Geneticfitparameter.levelfit == true) // Tobias 2013 Levelfit
    {
      ...
      std::cout << "std:" << floor(Daten.Geneticfitparameter.std_of_noise) << " ";

      // std::cout << "floor(Daten.Geneticfitparameter.level1)  :" << floor(Daten.Geneticfitparameter.level1) << std::endl;
      // std::cout << "floor(Daten.Geneticfitparameter.level2)  :" << floor(Daten.Geneticfitparameter.level2) << std::endl;
      // std::cout << "floor(Daten.Geneticfitparameter.std_of_noise)  :" << floor(Daten.Geneticfitparameter.std_of_noise) << std::endl;
    }
...
unsigned int GeneticFit_SimpleGA()
{
  ...
  // Efthymios 2025 baseline, amplitude, std of noise estimation
  if (Daten.Geneticfitparameter.levelfit == true)
  {
    map.add(Daten.Geneticfitparameter.bits, Daten.Geneticfitparameter.levelfit_min[1], Daten.Geneticfitparameter.levelfit_max[1]); // 1. baseline level
    map.add(Daten.Geneticfitparameter.bits, Daten.Geneticfitparameter.levelfit_min[2], Daten.Geneticfitparameter.levelfit_max[2]); // 2. amplitude level
    map.add(Daten.Geneticfitparameter.bits, Daten.Geneticfitparameter.levelfit_min[3], Daten.Geneticfitparameter.levelfit_max[3]); // 3. std of noise
  }
  ...
  ga.initialize();
  // std::cout << "convergence: " << Daten.Geneticfitparameter.nConvergence << std ::endl;

  // std::cout << "Daten.Geneticfitparameter.std_of_noise: " << Daten.Geneticfitparameter.std_of_noise << std::endl;
  ...
  if (Daten.Geneticfitparameter.levelfit == true) // Tobias 2013 Levelfit
    for (i = 1; i <= 10; i++)
    {
      ...
      if (Daten.Geneticfitparameter.state_conductance[i] == 1)
        Daten.Geneticfitparameter.stromvektor[i] = genome.phenotype(Setfile.get_no_rconst() + 1) + genome.phenotype(Setfile.get_no_rconst());
      ...
      Daten.a_fit.sigma = genome.phenotype(Setfile.get_no_rconst() + 2);
      ...
      Daten.Geneticfitparameter.std_of_noise = genome.phenotype(Setfile.get_no_rconst() + 2);
      Daten.a_fit.sigma = Daten.Geneticfitparameter.std_of_noise;
      std::cout << "genome.phenotype(Setfile.get_no_rconst() + 2)" << genome.phenotype(Setfile.get_no_rconst() + 2) << std::endl;
    }

  ergebnis = simulation(Daten.Fitparameter.multiplikator, 0, Ratenkonstanten, 0, genome.phenotype(Setfile.get_no_rconst() + 2));
...
void init_2Dfit()
{
  ...
  {
    Daten.a_fit.sigma = Daten.Geneticfitparameter.std_of_noise;
    Daten.a_fit.i_null = Setfile.get_zero_current() + Daten.Geneticfitparameter.level1;
    Daten.a_fit.i_channel = Daten.Geneticfitparameter.level2;
    Daten.a_fit.n_channels = Setfile.get_no_channels_total();
  }
  else
...
//***************************************************************************************************************************************
void generation_out_2Dfit(GASimpleGA &ga, GABin2DecGenome &genome)
{
  int i;
  std::ofstream logout(Daten.logfilename, std::ios::app);
  if (Daten.SimSettings.dosbox == true)
  ...
  if (Daten.Geneticfitparameter.levelfit == true) // Tobias 2013 Levelfit
  {
    ...
    std::cout << " fittet std_of_noise: " << Daten.Geneticfitparameter.std_of_noise;
  }
  std::cout << " ";
...
//***************************************************************************************************************************************
void summary_out_2Dfit(GASimpleGA &ga, GABin2DecGenome &genome)
{
  char textbuf[MaxTextLen];
  int i;
  int ii;
  double *Ratenkonstanten;
  Ratenkonstanten = new double[Setfile.get_no_rconst() + 4];
  int ampl[4096];
  int init_nr;
  ...
  if (Daten.Geneticfitparameter.levelfit == true) // Tobias 2013 Levelfit
  {
    ...
    Ratenkonstanten[Setfile.get_no_rconst() + 3] = genome.phenotype(Setfile.get_no_rconst() + 2);
    for (i = 1; i <= 10; i++)
    {
      if (Daten.Geneticfitparameter.state_conductance[i] == 0)
        Daten.Geneticfitparameter.stromvektor[i] = genome.phenotype(Setfile.get_no_rconst());
      if (Daten.Geneticfitparameter.state_conductance[i] == 1)
        Daten.Geneticfitparameter.stromvektor[i] = genome.phenotype(Setfile.get_no_rconst()) + genome.phenotype(Setfile.get_no_rconst() + 1);
      if (Daten.Geneticfitparameter.state_conductance[i] == 2)
        Daten.Geneticfitparameter.stromvektor[i] = genome.phenotype(Setfile.get_no_rconst() + 1);
    }
  }
  ...
  if (Daten.Geneticfitparameter.mpi_support == true)
  {
    for (i = 1; i <= Daten.Geneticfitparameter.nr_calc_mean_result; i++)
    {
      ...
      {
        init_nr++;
        MPI_Send(Ratenkonstanten, Setfile.get_no_rconst() + 4, MPI_DOUBLE, init_nr, 1, MPI_COMM_WORLD);
      }
      else // threreafter receive and send to same proc
      {
        ...
        MPI_Send(Ratenkonstanten, Setfile.get_no_rconst() + 4, MPI_DOUBLE, status.MPI_SOURCE, 1, MPI_COMM_WORLD);
      }
    }
    ...
    for (i = 1; i <= Daten.Geneticfitparameter.nr_calc_mean_result; i++)
    {
      erg[i] = simulation(Daten.Fitparameter.multiplikator, 0, Ratenkonstanten, 0, genome.phenotype(Setfile.get_no_rconst() + 2));
  ...
  if (Daten.Geneticfitparameter.levelfit == true) // Tobias 2013 Levelfit
  {
    ...
    std::cout << "best std_of_noise: " << Daten.Geneticfitparameter.std_of_noise << " ";
  }
  std::cout << std::endl
            << std::endl;
  ...
  if (Daten.Geneticfitparameter.levelfit == true) // Tobias 2013 Levelfit
  {
    ...
    result_out << Daten.Geneticfitparameter.std_of_noise << " ";
  }
  // setfile
...
void multiprocessor_master()
{
  ...
  {
    map.add(Daten.Geneticfitparameter.bits, Daten.Geneticfitparameter.levelfit_min[1], Daten.Geneticfitparameter.levelfit_max[1]); // 1. baseline level
    map.add(Daten.Geneticfitparameter.bits, Daten.Geneticfitparameter.levelfit_min[2], Daten.Geneticfitparameter.levelfit_max[2]); // 2. amplitude
    map.add(Daten.Geneticfitparameter.bits, Daten.Geneticfitparameter.levelfit_min[3], Daten.Geneticfitparameter.levelfit_max[3]); // 3. std of noise
  }
  ...
  if (Daten.Geneticfitparameter.fit_methode == 2) // amplitude histogram fit
    ga.minimize();
  if (Daten.Geneticfitparameter.fit_methode == 3)
  { // 2022 LLH is devided by amplitude error while CHI2 is multiplied
    if (Daten.calculate_error == 0)
  ...
  Ratenkonstanten = new double[Setfile.get_no_rconst() + 4]; // 1st termination signal for nodes
  Ratenkonstanten[0] = -1;
  for (i = 1; i < numprocs; i++)

    MPI_Send(Ratenkonstanten, Setfile.get_no_rconst() + 4, MPI_DOUBLE, i, 1, MPI_COMM_WORLD);
...
void multiprocessor_slave()
{
  ...
  while (1)
  {
    ...
    MPI_Recv(Ratenkonstanten,                                     /* message buffer */
             Setfile.get_no_rconst() + 4,                         /* one data item */
             MPI_DOUBLE,                                          /* of type double real */
             0,                                                   /* receive from any sender */
             1,                                                   /* any type of message */
             MPI_COMM_WORLD, /* default communicator */ &status); /* info about the received message */
    ...
    if (Daten.Geneticfitparameter.levelfit == true) // Tobias 2013 Levelfit
    {
      ...
      Daten.Geneticfitparameter.std_of_noise = Ratenkonstanten[Setfile.get_no_rconst() + 3];
      // std::cout << "Ratenkonstanten[Setfile.get_no_rconst() + 1]: " << Ratenkonstanten[Setfile.get_no_rconst() + 1] << std::endl;

      // std::cout << "Ratenkonstanten[Setfile.get_no_rconst() + 2]: " << Ratenkonstanten[Setfile.get_no_rconst() + 2] << std::endl;

      // std::cout << "Ratenkonstanten[Setfile.get_no_rconst() + 3]: " << Ratenkonstanten[Setfile.get_no_rconst() + 3] << std::endl;
      ...
      for (ii = 1; ii <= 10; ii++)
      {
        ...
        if (Daten.Geneticfitparameter.state_conductance[ii] == 1)
          Daten.Geneticfitparameter.stromvektor[ii] = Daten.Geneticfitparameter.level2 + Daten.Geneticfitparameter.level1;
    ...
    ergebnis[1] = simulation(Daten.Fitparameter.multiplikator, 0, Ratenkonstanten, 0, Ratenkonstanten[Setfile.get_no_rconst() + 3]);
  ...
  while (1) // calculate results
  {
    ...
    ergebnis[1] = simulation(Daten.Fitparameter.multiplikator, 0, Ratenkonstanten, 0, Daten.Geneticfitparameter.std_of_noise);
...
void multiprocessor_master_save_timeseries() // Tobias 2020
{
  ...
  {
    std::cout << " 2D-histograms" << std::endl;
    total_size = header_size + detector_levels_sim_e * dth_size;
    save_name.append("_header_");
    save_name.append(std::to_string(header_size));
    save_name.append("_2DtH_");
    save_name.append(std::to_string(detector_levels_sim_e * dth_size));
    save_name.append(".dat");
  }
  else
  {
  ...
  Ratenkonstanten[0] = -1;
  for (i = 1; i < numprocs; i++)
    MPI_Send(Ratenkonstanten, Setfile.get_no_rconst() + 4, MPI_DOUBLE, i, 1, MPI_COMM_WORLD); // termination
...
void multiprocessor_slave_save_timeseries() // Tobias 2020
{
  ...
  while (1)
  {
    ...
    if (Daten.SaveTimeseries.random_noise) // Tobias 2021 randomize noise
    {
      ...
      std::cout << "min SNR:" << Daten.SaveTimeseries.min_noise << " max SNR: " << Daten.SaveTimeseries.max_noise << " simulated SNR: " << random_SNR << " level1, Level2, max level: " << Daten.Geneticfitparameter.level1 << ", " << Daten.Geneticfitparameter.level2 << ", " << max_level << " random_sigma: " << random_sigma << std::endl;
    }
    ...
    std::string dummy2 = "";
    for (int i = 0; i < sizeof(dummy1) - 5; i++)
    { // Dateiendung vom string entfernen
    ...
    else if (Daten.SaveTimeseries.automated == 4) // transfer header + 2D multi level histogram to master
    {
      ...
      dth_size = detector_levels_sim_e * dth_count * sizeof(Daten.Dwell_2d_B.CO[0][0]);
      double levels[detector_levels_sim_e];

      for (int i = 0; i < int(detector_levels_sim_e / 2); i++)
      {
        levels[i] = i * (2.0 / detector_levels_sim_e) - 0.5;
        // std::cout<<"level_array: "<<levels[i]<<std::endl;
        // std::cout<<"value: "<<i * (2.0 / detector_levels_sim_e)<<std::endl;
        // std::cout<<"i: "<<i <<std::endl;
        // std::cout<<"(2.0 / detector_levels_sim_e): "<<(2.0 / detector_levels_sim_e)<<std::endl;
      }
      ...
      double *temp_buffer = new double[detector_levels_sim_e * dth_count];
      ...
      int i;
      for (i = 0; i < detector_levels_sim_e; i++) // detector_levels_sim_e levels
      {

        if (i < detector_levels_sim_e / 2)
        {
        ...
        else
        {
          ...
          Daten.a_fit.i_channel = original_amp - original_amp * levels[i - detector_levels_sim_e / 2];
          // std::cout<<"level_upper: "<<levels[i - detector_levels_sim_e/2]<<std::endl;
        }
