...
void TDaten::init()
{
  ...
  Daten.number_of_levels = 30; // for larger need to change exp 2D array size  TDwell_2d Dwell_2d_MA[20]; in daten.h // Efthymios 2025
}

void TDaten::reset_level()
{
...
int TDaten::get_data_from_file(char *Filename)
{
  ...
  if (Daten.import_asci_data == true)
  {
    ...
    for (j = 0; j < Anz_Samples; j++)
    {
      ...
      for (l = 0; l < 100; l++)
      {
        ...
        if ((importWert2 > 9) && (abfrage == true))
          break;
      }
      if (anzahl_ziffer == 1)
        Daten.Wert[j] = importWert1[0];
      if (anzahl_ziffer == 2)
        Daten.Wert[j] = importWert1[0] * 10 + importWert1[1];
      if (anzahl_ziffer == 3)
        Daten.Wert[j] = importWert1[0] * 100 + importWert1[1] * 10 + importWert1[2];
      if (anzahl_ziffer == 4)
        Daten.Wert[j] = importWert1[0] * 1000 + importWert1[1] * 100 + importWert1[2] * 10 + importWert1[3];
      if (anzahl_ziffer == 5)
        Daten.Wert[j] = importWert1[0] * 10000 + importWert1[1] * 1000 + importWert1[2] * 100 + importWert1[3] * 10 + importWert1[4];

      if (Daten.Wert[j] > AD_towert)
        AD_towert = Daten.Wert[j];
      if (Daten.Wert[j] < AD_fromwert)
        AD_fromwert = Daten.Wert[j];
...
void TDaten::load(char *Filename)
{
  /*2019
  // jobfile laden
...
void TDaten::store(char *Filename)
{
  // jobfile speichern
...
void TDaten::markeddatafit()
{
  // still under construction (toby)
}

void TDaten::PlotItem(TDwell &Item, int &Startindex, int &i, int &k, TTargetWeight *weight, double &Kreuz, double &old)
{
  /*2019
...
bool TDaten::ProcessedText(int i)
{
...
void TDaten::str_filter(bool clear)
{
  /*2019
...
void TDaten::histogram(Tampl_histo_type a)
{
...
void TDaten::ampl_min_max(Tampl_histo_type a, int &min, int &max)
{
...
void TDaten::amp_grow(Tampl_histo_type a, int max)
{
  /*2019
  short  i;
  short  j;
...
void TDaten::plot_strom(double strom)
{
  /*2019
  short  i;
  short  window_height;
...
bool TDaten::checkampl(Tampl_fit_type ampl)
{
...
void TDaten::show_ampl_histo(Tampl_histo_type a)
{
  /*2019
...
void TDaten::calc_fit(Tampl_fit_type &af, Tampl_histo_type histo, int min, int max)
{
  int i, k;
...
void TDaten::simplex(Tampl_fit_type &af, Tampl_histo_type histo, int min, int max)
{
  int i;               //,j,k;
...
void TDaten::plot_levels_and_fit(Tampl_fit_type &a, int max_h, double stromrand, double heightrand) // Tobias 2017 weg: =0
{
  /*2019
...
void TDaten::calc_ahisto()
{
  ...
  case normal:
    ...
    {
      if (k >= (Anz_Samples - Settings.LengthOfAvarageFilter()))
        break;
      awert = 0;
      for (l = 0; l < Settings.LengthOfAvarageFilter(); l++)
      {
  ...
  case gleitend:
    for (k = x; k <= x + Settings.SamplesToProcess; k++)
    {
      if (k >= (Anz_Samples - vor))
        break;
      awert = 0;

      for (l = x - vor; l < x + nach; l++)
      {
        if ((l >= 0) && (l < Anz_Samples))
          awert += Wert[l];
      }

      laenge = Settings.LengthOfAvarageFilter();
      if (x - vor < 0)
        laenge += (x - vor); // x-vor ist negativ!
      if (x + nach >= Daten.Anz_Samples)
        laenge -= (x + nach - Daten.Anz_Samples + 1);
      awert /= laenge;
