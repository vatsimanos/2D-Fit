...
short TSettings::StackedWindows()
{
  if (Zoom)
  {
...
short TSettings::LengthOfAvarageFilter()
{
  if (Zoom)
  {
    if (Zoomed.AvarageFilter == keine)
      return 1;
    return Zoomed.LengthOfAvarageFilter;
  }
  else
  {
    if (UnZoomed.AvarageFilter == keine)
      return 1;
    return UnZoomed.LengthOfAvarageFilter;
  }
}

void TSettings::load(std::istream &in) // std::istream& in
{
...
void TSettings::store(std::ostream &out)
{
  /*2019
...
std::istream &operator >> (std::istream & in, TSettings &Settings)
{
  ...
  if (!strncmp(buffer, "#settings", 9)) // 2019 strlwr removed
  {
    ...
    while (in.get(c))
      if (c == '\n')
        break;
    in >> Daten.Geneticfitparameter.startwertmin[11];
    while (in.get(c))
      if (c == '\n')
        break;
    in >> Daten.Geneticfitparameter.startwertmax[11];
    while (in.get(c))
      if (c == '\n')
        break;
    in >> Daten.Geneticfitparameter.startwertmin[12];
    while (in.get(c))
      if (c == '\n')
        break;
    in >> Daten.Geneticfitparameter.startwertmax[12];
    while (in.get(c))
      if (c == '\n')
        break;
    in >> Daten.Geneticfitparameter.startwertmin[13];
    while (in.get(c))
      if (c == '\n')
        break;
    in >> Daten.Geneticfitparameter.startwertmax[13];
    while (in.get(c))
      if (c == '\n')
        break;
    in >> Daten.Geneticfitparameter.startwertmin[14];
    while (in.get(c))
      if (c == '\n')
        break;
    in >> Daten.Geneticfitparameter.startwertmax[14];
    while (in.get(c))
      if (c == '\n')
        break;
    ...
    while (in.get(c))
      if (c == '\n')
        break;
    in >> Daten.Geneticfitparameter.levelfit_min[3];
    while (in.get(c))
      if (c == '\n')
        break;
    in >> Daten.Geneticfitparameter.levelfit_max[3];
    while (in.get(c))
      if (c == '\n')
        break;
...
void TSprung::load(std::istream &in)
{
...
void TSprung::store(std::ostream &out)
{
...
void TSprung::plot(short strom_min, short strom_max)
{
  /*2019
  // aus bigcolle.pas
  short   i,i_window = 0;
...
void TSprung::reset(bool draw) // Tobias: = true weg
{
  /*2019
...
void TSprung::calcStD(double level)
{
  int i;
  double l2;
...
void TDwell::load(std::istream &in)
{
  in.read((char *)&x, sizeof(x));
  in.read((char *)&y, sizeof(y));
}

void TDwell::store(std::ostream &out)
{
  out.write((char *)&x, sizeof(x));
  out.write((char *)&y, sizeof(y));
}
...
void Tniveau::load(std::istream &in)
{
...
void Tniveau::store(std::ostream &out)
{
