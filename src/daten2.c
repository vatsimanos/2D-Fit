...
void TDaten::InitDwellTimeHisto(int n_channels, int n_extra_channels, bool neu)
{
  // TDaten
...
void TDaten::hinkley(detector_type d)
{
...
void TDaten::HOHD()
{
  ...
  // std::cout << "HOHD: a_fit.i_null :" << a_fit.i_null << std::endl;
  // std::cout << "HOHD: a_fit.i_channel  :" << a_fit.i_channel << std::endl;
  // std::cout << "HOHD: a_fit.sigma  :" << a_fit.sigma << std::endl;
...
void TDaten::HOHD_S(double lambda)
{
...
void TDaten::create_neighbors(Tneighbor_type &n, int lastjump, int open, int extra_open)
{
  int candi, i, k, g;
  bool allowed = false;
...
bool TDaten::flush_Sprung(detector_type d,
                          ...
                          int n_extra_open_new)
{
  ...
  if (Spruenge_size < Spruenge.max_size())
  {
    ...
    {
      if (Spruenge.empty())
        old_index = Settings.SamplesToSkip; // Startwert wird auf Ausschnittsstartpunkt gesetzt
...
void TDaten::separatechannels()
{
  ...
  for (unsigned short i = 0; i < channels; i++)
    close_channels.push_back(&SingleChannels[i]);
  open_channels.clear();
...
void TDaten::SaveDwell(TDwell &Item, std::ofstream &out)
{
...
void TDaten::saveDwellTimeHisto(char *name)
{
...
void TDaten::saveDetectedLevelHisto(char *name)
{
  int sum;
  int i, j, k, l;
  ...
  for (i = 0; i <= a_fit.n_channels; i++)
  {
    for (j = 0; j <= a_fit.n_extra_channels; j++)
    {
      ...
      if (sum != 0)
      {
        if (l == 0)
        {
          Textfile << std::setw(10) << ((double)(Daten.Settings.SamplesToProcess - sum + Daten.result.detected_level_histo[l]) / (double)Daten.Settings.SamplesToProcess);
        }
        else
        {
  ...
  Textfile << std::endl
           << "Offenwahrscheinlichkeiten:" << std::endl;
...
void TDaten::saveTransitionMatrix(char *name)
{
  int i, j;
...
void TDaten::clearDetectedLevels()
{
  // waere  wohl besser bei Tresult aufgehoben
  int i;
  for (i = 0; i <= map; i++)
    result.detected_level_histo[i] = 0;
}

void TDaten::clearTransitions()
{
...
TDaten::calcStD(int n_channel, int n_extra_channel)
{
  ...
  // Spruenge:=nil;
  level = a_fit.i_null + a_fit.i_channel * HText.level + a_fit.i_extra_channel * HText.sublevel;

  switch (HText.detector)
  {
  case _HOHD:
    level = a_fit.i_null + a_fit.i_channel * HText.level;
    if (!HOHD_Spruenge.empty())
      HText.sigma_of_channel = TSprung::calcSD(level, HOHD_Spruenge);
    return HText.sigma_of_channel;
    break;
  case _SHD:
    if (!SHD_Spruenge.empty())
      HText.sigma_of_channel = TSprung::calcSD(level, SHD_Spruenge);
    return HText.sigma_of_channel;
    break;
  case _HOSHD:
    if (!HOSHD_Spruenge.empty())
      HText.sigma_of_channel = TSprung::calcSD(level, HOSHD_Spruenge);
    return HText.sigma_of_channel;
    break;
  case _SHD_AH:
    break; // Tobias 2013
...
void TDaten::StartIndex(TDwell &Dwell, int &index, double &x, double &y)
{
  int i = 0;
  ...
  if (Dwell.y > y)
  {
    ...
    while (&QuenchedDwellTime[i] != &Dwell && i < (int)QuenchedDwellTime.size())
      i++;
    index = i; // QuenchedDwellTime.IndexOf(Dwell);
  }
}

int TDaten::CalcStartIndex()
{
  int index;
  double x, y;
  std::vector<TDwell>::iterator iter;
...
void TDaten::axes(double mx, double my, double xs, double ys)
{
  /*2019
...
void TDaten::generate_gnu_plot(char *name)
{
...
void TDaten::exec_gnuplot(char *name)
{
  /*
...
void TDaten::algAbspeichern(VecD lambda[], VecD ampli[], std::vector<TUntermatrix> &Dimension)
{
  int i, j;
  int start, stop, dim;
  double l;
