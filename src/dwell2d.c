...
void TDwell_lb::calcrange(short &log_min_close, short &log_max_close,
                          short &log_min_open, short &log_max_open)
{
  ...
  if (!Daten.HOHD_Spruenge.empty())
  {
    ...
    if (max_c != 0)
      log_max_close = (short)ceil(log10(max_c));
    if (min_c != 0)
      log_min_close = (short)floor(log10(min_c));
    if (max_o != 0)
      log_max_open = (short)ceil(log10(max_o));
    if (min_o != 0)
      log_min_open = (short)floor(log10(min_o));
  }
  else
  {
...
void TDwell_lb::init(char *aname, unsigned short abins) // Tobias 2017 weg: =0
{
...
void TDwell_lb::init(char *aname,
...
void TDwell_lb::GNUplot(bool dosqrt) // Tobias 2017 weg: =true
{
  /*2019
    char cmd[] = "wgnuplot";
...
void TDwell_lb::out(std::ostream &out)
{
...
void TDwell_lb::in(std::istream &in)
{
...
void TDwell_lb::load(std::istream &in)
{
...
void TDwell_lb::store(std::ostream &out)
{
...
void TDwell_1d::init(char *aname, unsigned short abins) // Tobias weg: =0
{
...
void TDwell_1d::init(char *aname,
                     short log_min_close, short log_max_close,
                     short log_min_open, short log_max_open,
...
void TDwell_1d::calc(std::vector<TSprung> *vec_ptr, double maxevents) // Tobias weg: =0
{
  std::vector<TSprung>::iterator iter1;
  std::vector<TSprung>::iterator iter2;
...
void TDwell_1d::GNUplot(bool dosqrt) // Tobias 2017 weg: = true
{
  double log_laenge;
  ...
  for (unsigned short i = 0; i < bins_open(); i++)
  {
    log_laenge = ((i * (max_open_log() - min_open_log()) / (double)bins_open() + min_open_log()) +
                  ((i + 1) * (max_open_log() - min_open_log()) / (double)bins_open() + min_open_log())) /
                 2;
  ...
  for (unsigned short i = 0; i < bins_close(); i++)
  {
    log_laenge = ((i * (max_close_log() - min_close_log()) / (double)bins_close() + min_close_log()) +
                  ((i + 1) * (max_close_log() - min_close_log()) / (double)bins_close() + min_close_log())) /
                 2;
...
void TDwell_1d::load(std::istream &in)
{
  // zum laden aus setfile
  TDwell_lb::load(in);
...
void TDwell_1d::store(std::ostream &out)
{
  // abspeichern ins jobfile
  TDwell_lb::store(out);
...
void TDwell_2d::init(char *aname, unsigned short abins) // Tobias 2017 weg: = 0
{
...
void TDwell_2d::init(char *aname,
                     short log_min_close, short log_max_close,
                     short log_min_open, short log_max_open,
                     unsigned short abins) // Tobias 2017 weg: = 0
{
...
void TDwell_2d::calc(std::vector<TSprung> *vec_ptr, double maxevents) // Tobias 2017 weg: = 0
{
  std::vector<TSprung>::iterator iter1;
  std::vector<TSprung>::iterator iter2;
...
void TDwell_2d::calc_dependency()
{
...
void TDwell_2d::addjumps(TDwell_1d *dwell_1d, double maxevents) // Tobias 2017 weg: =NULL  = 0
{
  ...
  {
    if ((*sprung_iter).C_2() > channels)
      channels = (*sprung_iter).C_2();
    if ((*sprung_iter).U_2() > subchannels)
      subchannels = (*sprung_iter).U_2();
  }
  calc(&Daten.HOHD_Spruenge, maxevents);
  if (dwell_1d != NULL)
    dwell_1d->calc(&Daten.HOHD_Spruenge, maxevents);
...
void TDwell_2d::Hinkley_Log(TDwell_1d *dwell_1d, double maxevents) // Tobias 2017 weg: = NULL  = 0
{
  std::vector<TSprung>::iterator sprung_iter;
  int i, j;
...
void TDwell_2d::GNUplot(bool dosqrt) // Tobias 2017 weg: = true
{
  double log_laenge1, log_laenge2;
  ...
  for (unsigned short i = 0; i < bins_close(); i++)
  {
    ...
    for (unsigned short j = 0; j < bins_open(); j++)
    {
      log_laenge2 = ((j * (max_open_log() - min_open_log()) / (double)bins_open() + min_open_log()) +
                     ((j + 1) * (max_open_log() - min_open_log()) / (double)bins_open() + min_open_log())) /
                    2;
...
TDwell_2d::lnlikelihood(TDwell_2d &simul)
{
  ...
  {
    // std::cout << "simul.events " << simul.events() << std::endl;
    ...
    for (j = 0; j < (log_max_close - log_min_close) * bins_per_log(); j++)
    {
      ...
      { // helper = helper + CO[j][k];
        ...
        if ((simul.CO[j + simul_close_off][k + simul_open_off] != 0) && (CO[j + close_off][k + open_off] != 0))
        {
          ...
          // std::cout<<"value: "<<log((simul.CO[j+simul_close_off][k+simul_open_off])/  simul.events()  * events() ) * CO[j+close_off][k+open_off] * dummy_factor<<std::endl;
          // std::cout<<"dummy_factor: "<<dummy_factor<<std::endl;
          // std::cout<<"log((1 + simul.CO[j+simul_close_off][k+simul_open_off]): "<<log(1 + simul.CO[j+simul_close_off][k+simul_open_off])<<std::endl;																						  //Magleby & Weiss have simulated a defined number of events equalling the measured data. Otherwise the LLh does not work, because there is no penalty for to much events
        ...
        for (j = 0; j < (log_max_close - log_min_close) * bins_per_log(); j++)
        {
          for (k = 0; k < (log_max_open - log_min_open) * bins_per_log(); k++)
          {
            ...
            if (k > 0)
              ...
              ymin = k;
            if (j < ((log_max_close - log_min_close) * bins_per_log() - 1))
              xmax = j + 1;
            else
              xmax = j;
            if (k < ((log_max_open - log_min_open) * bins_per_log() - 1))
              ymax = k + 1;
            else
              ymax = k;
          ...
          for (j = 0; j < (log_max_close - log_min_close) * bins_per_log(); j++)
          {
            for (k = 0; k < (log_max_open - log_min_open) * bins_per_log(); k++)
            {
              ...
              if (ymin < 0)
                ymin = 0;
              xmax = j + 2;
              if (xmax > ((log_max_close - log_min_close) * bins_per_log() - 1))
                xmax = (log_max_close - log_min_close) * bins_per_log() - 1;
              ymax = k + 2;
              if (ymax > ((log_max_open - log_min_open) * bins_per_log() - 1))
                ymax = (log_max_open - log_min_open) * bins_per_log() - 1;
...
void TDwell_2d::load(std::istream &in)
{
  // zum laden aus setfile
  TDwell_lb::load(in);
...
void TDwell_2d::store(std::ostream &out)
{
  // abspeichern ins jobfile
  TDwell_lb::store(out);
